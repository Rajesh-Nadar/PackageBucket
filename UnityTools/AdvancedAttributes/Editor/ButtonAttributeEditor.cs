...
namespace UnityTools.AdvancedAttributes
{
    ...
    public class ButtonAttributeEditor : Editor
    {
        ...
        Dictionary<Type, ParameterDrawer> typeDrawer = new()
        {
        ...
        Dictionary<Type, string> typeDisplayName = new()
        {
        ...
        void CreateEditorButtonStates(MethodInfo[] methods)
        {
            ...
            foreach (var methodInfo in methods)
            {
                ...
                editorButtonStates[methodIndex].name = attribute.ButtonName;
        ...
        void DrawButtonforMethod(object[] invokationTargets, MethodInfo methodInfo, EditorButtonState state)
        {
            ...
            var i = (parameters.Length > 0) ? 10 : 0;
            var foldoutRect = EditorGUILayout.GetControlRect(GUILayout.Width(i));
            foldoutRect.x += 12;

            if (parameters.Length > 0)
                state.opened = EditorGUI.Foldout(foldoutRect, state.opened, "");

            bool clicked = GUILayout.Button(MethodDisplayName(methodInfo, state.name), GUI.skin.label, GUILayout.ExpandWidth(true));
            ...
            if (clicked)
            {
                foreach (var invokationTarget in invokationTargets)
                {
                    ...
                    {
                        monoTarget.StartCoroutine(enumerator);
                    }
